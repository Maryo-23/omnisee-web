...
export default function PanoramaViewer({ post, author, currentUser, darkMode, onClose, onDelete }: PanoramaViewerProps) {
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', background: darkMode ? '#0a0a0a' : '#ffffff' }}
      onClick={onClose}
    >
      {/* Left - Viewer */}
      <div
      ...
      >
        ...
        <div style={{ position: 'absolute', top: 24, left: 24, display: 'flex', flexDirection: 'column', gap: 12, zIndex: 20, pointerEvents: 'none' }}>
          ...
          <button onClick={() => navigator.clipboard.writeText(window.location.href)} style={{ width: 44, height: 44, borderRadius: '50%', border: 'none', background: 'rgba(0,0,0,0.6)', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, pointerEvents: 'auto' }} title="Share">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8" /><polyline points="16 6 12 2 8 6" /><line x1="12" y1="2" x2="12" y2="15" /></svg>
          </button>
        </div>

        {/* Bottom Left Map */}
        <div style={{ position: 'absolute', bottom: 24, left: 24, width: 140, height: 100, borderRadius: 10, overflow: 'hidden', zIndex: 20, boxShadow: '0 4px 12px rgba(0,0,0,0.3)', pointerEvents: 'none' }}>
          <div style={{ width: '100%', height: '100%', background: '#e5e7eb', position: 'relative' }}>
            ...
            <div style={{ position: 'absolute', bottom: 4, left: 4, fontSize: 10, color: '#4b5563', background: 'rgba(255,255,255,0.8)', padding: '2px 4px', borderRadius: 4 }}>{post?.location || 'Map'}</div>
          </div>
        </div>

        {/* Bottom Center Controls */}
        <div style={{ position: 'absolute', bottom: 24, left: '50%', transform: 'translateX(-50%)', display: 'flex', alignItems: 'center', gap: 4, zIndex: 20, background: 'rgba(0,0,0,0.75)', borderRadius: 10, padding: '6px 10px', pointerEvents: 'none' }}>
          <button style={{ width: 36, height: 36, borderRadius: 8, border: 'none', background: 'transparent', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'auto' }} title="Fullscreen">
          ...
          <button onClick={() => navigator.clipboard.writeText(window.location.href)} style={{ width: 36, height: 36, borderRadius: 8, border: 'none', background: 'transparent', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'auto' }} title="Copy link">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" /><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" /></svg>
          </button>
          <button style={{ width: 36, height: 36, borderRadius: 8, border: 'none', background: 'transparent', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'auto' }} title="Gyroscope">
          ...
          <button style={{ width: 36, height: 36, borderRadius: 8, border: 'none', background: 'transparent', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'auto' }} title="Zoom">
      ...
      {/* Right Sidebar */}
      <div style={{ width: 400, minWidth: 360, maxWidth: 440, display: 'flex', flexDirection: 'column', background: darkMode ? '#0f0f0f' : '#ffffff', color: darkMode ? '#ffffff' : '#111827', borderLeft: `1px solid ${darkMode ? '#262626' : '#e5e7eb'}` }} onClick={(e) => e.stopPropagation()}>
        {/* Profile Header */}
        <div style={{ padding: '20px 24px', borderBottom: `1px solid ${darkMode ? '#262626' : '#e5e7eb'}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <img src={avatarUrl(author)} alt="" style={{ width: 48, height: 48, borderRadius: '50%', objectFit: 'cover', border: `2px solid ${darkMode ? '#333' : '#e5e7eb'}` }} />
            <div>
          ...
          <button style={{ width: 32, height: 32, borderRadius: '50%', border: 'none', background: 'transparent', color: darkMode ? '#a1a1aa' : '#6b7280', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
          <div style={{ color: darkMode ? '#a1a1aa' : '#6b7280', fontSize: 13, marginBottom: 6 }}>Post Location : {post?.location || 'Unknown location'}</div>
          <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 6, lineHeight: 1.4 }}>{post?.caption || 'Untitled'}</div>
          <div style={{ color: darkMode ? '#a1a1aa' : '#9ca3af', fontSize: 13, marginBottom: 16 }}>{timeAgo(post?.created_at)}</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 20, paddingBottom: 16, borderBottom: `1px solid ${darkMode ? '#262626' : '#e5e7eb'}` }}>
            ...
            <button onClick={handleLike} style={{ display: 'flex', alignItems: 'center', gap: 6, color: liked ? '#ef4444' : (darkMode ? '#a1a1aa' : '#6b7280'), fontSize: 14, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
          ...
          <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 16 }}>Comments ({comments.length})</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            {comments.length === 0 && (
              <div style={{ color: darkMode ? '#71717a' : '#9ca3af', fontSize: 14, textAlign: 'center', padding: '20px 0' }}>No comments yet. Be the first to comment!</div>
            )}
            {comments.map((comment: any) => (
              <div key={comment.id} style={{ display: 'flex', gap: 12 }}>
                <img src={avatarUrl({ username: comment.username, avatar_url: comment.avatarUrl })} alt="" style={{ width: 40, height: 40, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ marginTop: 4, fontSize: 14, lineHeight: 1.5, wordBreak: 'break-word' }}>{comment.text}</div>
        ...
        <div style={{ padding: '14px 24px', borderTop: `1px solid ${darkMode ? '#262626' : '#e5e7eb'}`, background: darkMode ? '#0f0f0f' : '#ffffff' }}>
          {currentUser ? (
            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              <input type="text" value={commentText} onChange={(e) => setCommentText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleComment()} placeholder="Write a comment..." style={{ flex: 1, padding: '10px 14px', borderRadius: 8, border: `1px solid ${darkMode ? '#333' : '#e5e7eb'}`, background: darkMode ? '#1a1a1a' : '#f9fafb', color: darkMode ? '#fff' : '#111', fontSize: 14, outline: 'none' }} />
              <button onClick={handleComment} disabled={!commentText.trim()} style={{ padding: '10px 18px', borderRadius: 8, border: 'none', background: '#6366F1', color: 'white', fontSize: 14, fontWeight: 600, cursor: commentText.trim() ? 'pointer' : 'not-allowed', opacity: commentText.trim() ? 1 : 0.5 }}>Post</button>
            </div>
          ) : (
            <button onClick={onClose} style={{ width: '100%', padding: '12px', borderRadius: 8, border: 'none', background: '#6366F1', color: 'white', fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>Log in to comment</button>
          )}
        </div>
      </div>
    </div>
